...
internal class DateMatcher : IMatcher
{
    ...
    private readonly Regex dateWithNoSeparator = new("^\\d{4,8}$", RegexOptions.Compiled | RegexOptions.IgnorePatternWhitespace);

    private readonly Regex dateWithSeparator = new(
        @"^( \d{1,4} )    # day or month
               ( [\s/\\_.-] ) # separator
               ( \d{1,2} )    # month or day
               \2             # same separator
    ...
    public IEnumerable<Matches.Match> MatchPassword(string password)
    {
        ...
        for (var i = 0; i <= password.Length - 4; i++)
        {
            ...
            {
                var dateMatch = dateWithNoSeparator.Match(password); // Slashless dates
                ...
                static int Metric(LooseDate c) => Math.Abs(c.Year - ReferenceYear);
        ...
        for (var i = 0; i <= password.Length - 6; i++)
        {
            for (var j = 6; i + j <= password.Length; j++)
            {
                ...
                var match = dateWithSeparator.Match(token);
